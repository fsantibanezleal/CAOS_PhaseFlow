    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p>
            {es
              ? 'El peor caso retenido es 0,344 del plan exacto, y un peor caso es una nota al pie hasta que se puede decir CUANDO. La hipotesis obvia era el arquetipo: las seis fallas de entrenamiento son core_halo. No sobrevive, tres de las cinco fallas retenidas son vein. Un arquetipo es una ETIQUETA y el barrido tiene cuatro; un patron que calza con una etiqueta en seis casos calza con muchas cosas.'
              : 'The worst held-out case is 0.344 of the exact plan, and a worst case is a footnote until you can say WHEN. The obvious hypothesis was the archetype: all six training failures are core_halo. It does not survive, three of the five held-out failures are vein. An archetype is a LABEL and the sweep has four of them; a pattern that lines up with a label across six cases lines up with a lot of things.'}
          </p>
          <p>
            {es
              ? 'Lo que si sobrevive es el ESCENARIO, y es la lectura que tiene mecanismo: con descuento alto el valor de un plan depende del momento exacto en que se saca cada bloque, y a un sustituto entrenado y evaluado como CLAVE DE ORDEN se le pide precisamente lo que menos tiene. La regla que se embarca es tasa de descuento >= 0,15, elegida por RECALL: retenida atrapa las cinco fallas de cinco, marca 40 por ciento de los casos, y ningun caso NO marcado perdio mas de 7 por ciento. La regla por arquetipo no puede decir eso, y es la que sugerian los datos de entrenamiento.'
              : 'What survives is the SCENARIO, and it is the reading with a mechanism: under heavy discounting the value of a plan depends on precisely when each block is taken, and a surrogate trained and scored as a SORT KEY is being asked for exactly what it has least of. The shipped rule is discount rate >= 0.15, chosen for RECALL: held out it catches five failures of five, flags 40 percent of cases, and no unflagged case lost more than 7 percent. The archetype rule cannot say that, and it is the one the training data pointed at.'}
          </p>
          <Callout variant="honest" title={es ? 'La advertencia va junto al selector' : 'The warning belongs next to the selector'}>
            {es
              ? 'Una advertencia en una pagina de metodologia es una salvedad; el mismo hecho donde se elige el metodo es una guarda. El peldano aprendido lleva su bandera en el artefacto y el selector la marca. Y una salvedad sobre la salvedad: los numeros retenidos de esta regla son honestos pero no limpios, porque el mismo conjunto retenido que refuto la regla por arquetipo motivo elegir esta; haria falta una tercera particion para llamarlos insesgados.'
              : "A warning on a methodology page is a caveat; the same fact where the method is chosen is a guard. The learned rung carries its flag in the artifact and the selector marks it. And a caveat about the caveat: this rule's held-out numbers are honest but not clean, because the same held-out set that refuted the archetype rule also motivated choosing this one; a third split would be needed to call them unbiased."}
          </Callout>
