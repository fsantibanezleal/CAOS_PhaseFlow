...
- a bound surrogate with a 1.40 percent held-out error that predicted more capacity LOWERING a bound,
  because the training sweep ran rate and capacity together at a correlation of -0.735
  ([10](methods/10_when_the_surrogate_fails.md))
- and the same confound producing two different wrong answers about when the learned rung fails, one
  of which this repo had already written down ([10](methods/10_when_the_surrogate_fails.md))
- two copies of `react-router`, which crashed every route while the HTTP check reported 200
  (the deployment note kept with the operations toolbox)
