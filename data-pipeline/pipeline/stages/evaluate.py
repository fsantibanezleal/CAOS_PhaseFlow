...
def run_ensemble(instance, results, *, n: int = 12, sigma: float = 0.25, seed: int = 31) -> dict:
    ...
    if instance.n_blocks > 16_000:
        return {
            "ran": False,
            "reason": (
                f"{instance.n_blocks:,} blocks: each realisation is a full re-solve and the certified "
                "bound is a sequence of maximum closures in pure Python, so the ensemble is above "
                "budget here. The risk readout is shown on the smaller cases."
            ),
        }
    if instance.n_blocks > 8_000:
        n = max(6, n // 2)
    ...
    for j in range(ens.n_realisations):
        ...
        rj, _ = ob.solve_cpit(inst_j, instance.precedence)
